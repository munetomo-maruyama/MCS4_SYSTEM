...
// Rev.03 2025.07.06 M.Maruyama ROM Size is 128KB
//                              Generate Byte Lane 1
//-----------------------------------------------------------
...
{
    uint32_t addr;
    uint32_t addr_verilog;
    //
    addr_verilog = 0;
    for (addr = addr_min + 1; addr <= addr_max; addr = addr + 4)
    {
        printf ("\n@%08X %02X", addr_verilog, ROM[addr]);
        addr_verilog++;
    }
}
