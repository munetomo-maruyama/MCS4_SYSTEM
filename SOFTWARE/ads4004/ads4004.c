...
//
uint16_t STACK[4];   // PC and Stack
uint32_t STACK_CUR;  // Current Stack Pointer
...
uint8_t ROM[MAXROM];
uint32_t pc_max = 0;
static volatile uint32_t ctrl_c = 0;
...
uint32_t Put_Error(FILE *fp, uint32_t result, uint32_t pass, uint32_t line)
{
    ...
    switch (result)
    {
        case RESULT_NO         : {fprintf(fp, "ERROR: Illegal Item"); error = 1; break;}
        case RESULT_OK         : {error = 0; break;}
        case RESULT_END        : {error = 0; break;}
        case RESULT_ALLOCERR   : {fprintf(fp, "ERROR: Internal Memory Allocation Error"); error = 1; break;}
        case RESULT_UNDEFINED  : {if (pass) fprintf(fp, "ERROR: Syntax Error"); error = pass; break;}
        case RESULT_REDEFINED  : {fprintf(fp, "ERROR: Redefined Label"); error = 1; break;}
        case RESULT_DIVZERO    : {fprintf(fp, "ERROR: Div by Zero"); error = 1; break;}
        case RESULT_NOMNEMONIC : {fprintf(fp, "ERROR: Unknown Mnemonic"); error = 1; break;}
        case RESULT_NOOPERAND  : {fprintf(fp, "ERROR: Illegal Operand"); error = 1; break;}
        case RESULT_ROMOVERFLOW: {fprintf(fp, "ERROR: ROM Overflow"); error = 1; break;}
        case RESULT_ILGLTARGET : {fprintf(fp, "ERROR: Illegal Jump Target"); error = 1; break;}
        case RESULT_INTERNALERR: {fprintf(fp, "ERROR: Internal Error"); error = 1; break;}
        case RESULT_ILGLHEX    : {fprintf(fp, "ERROR: Illegal Hex Format"); error = 1; break;}
        case RESULT_TRUNCATE   : {fprintf(fp, "WARNING: Ignored Upper Bits of Immediate Value"); warning = 1; break;}
        default : {fprintf(fp, "ERROR: Something Unknown Happened"); error = 1; break;}
    }
    if (error | warning) fprintf(fp, " in line %d.\n", line);
    return error;
}
...
void Put_Assemble_List(FILE *fp, uint8_t *pline, uint32_t pc_bgn, uint32_t pc_nxt)
{
    ...
    fprintf(fp, "%03X ", pc_bgn);
    if (pc_bgn == pc_nxt) {fprintf(fp, "     "); pc = pc_bgn;}
    else if ((pc_bgn + 1) == pc_nxt) {fprintf(fp, "%02X   "  , ROM[pc_bgn]); pc = pc_bgn + 1;}
    else if ((pc_bgn + 2) <= pc_nxt) {fprintf(fp, "%02X %02X", ROM[pc_bgn], ROM[pc_bgn + 1]); pc = pc_bgn + 2;}
    fprintf(fp, "   %s", pline);
    if ((pc_bgn + 2) < pc_nxt)
    {
        count = 0;
        for (pc = pc_bgn + 2; pc < pc_nxt; pc++)
        {
            if (count == 0)
            {
                fprintf(fp, "%03X %02X", pc, ROM[pc]);
                if ((pc + 1) == pc_nxt) fprintf(fp, "\n");
            }
            if (count == 1) fprintf(fp, " %02X\n", ROM[pc]);
            count = (count + 1) % 2;
        }
    }
}
    ...
    for (addr = 0; addr < pc_max; addr++)
    {
        for (i = 0; i < LABELROOT; i++)
        {
            for (plabel = Label_Root[i]; plabel != NULL; plabel = plabel->next)
            {
                ...
                {
                    fprintf(fp, "%-8s = 0x%04X\n", plabel->name, plabel->value);
                }
            }
        }
    }
    fprintf(fp, "-----End of Label Table-----\n");
}
...
void Put_Hex_Format(FILE *fp)
{
    ...
    for (pc = 0; pc < pc_max; pc = pc + 16)
    {
        ...
        fprintf(fp, ":%02X%04X00", rest, pc);
        checksum = rest + (pc >> 8) + (pc & 0x00ff);
        for (i = 0; i < rest; i++)
        {
            fprintf(fp, "%02X", ROM[pc+i]);
...
uint32_t Get_Hex_Format(FILE *fp)
{
    ...
    {
        // Get a Line
        if (fgets(hexline, MAXLEN_LINE, fp) == NULL) return;
        pline = hexline;
        // Start Code
...
{
    uint32_t pc;
    fprintf(fp, "----ROM Contents pc_max=%03x\n", pc_max);
    for (pc = 0; pc < pc_max; pc++)
    {
        if ((pc % 16) == 0) printf("%03x ", pc);
        fprintf(fp, "%02x ", ROM[pc]);
        if ( ((pc > 0) && (((pc + 1) % 16) == 0)) || ((pc + 1) == pc_max) ) printf("\n");
    }
    fprintf(fp, "----End of ROM Contents\n");
}
...
void Put_Sim_CPU_Status(FILE *fp)
{
    ...
    addr = PC;
    String_Disasm(str, &addr);
    Sim_fprintf(fp, "  PC=%03X ACC=%01X CY=%01X", PC, ACC, CY);
    Sim_fprintf(fp, " R0-15=");
    for (i = 0; i < 16; i++) Sim_fprintf(fp, "%01X", R[i]);
    Sim_fprintf(fp, " DCL=%01X SRC=%02X", REG_DCL, REG_SRC);
    Sim_fprintf(fp, "  (%s)\n", str); // disasm result
}
...
void Put_RAM_Status(FILE *fp, uint32_t bank)
{
    ...
    for (chip = 0; chip < 4; chip++)
    {
        for (reg = 0; reg < 4; reg++)
        {
            ...
            Sim_fprintf(fp, "Bank=%01x Chip=%01x Reg=%01x Addr=%03x ", bank, chip, reg, adrm);
            for (ch = 0; ch < 16; ch++)
            {
                Sim_fprintf(fp, " %01x", RAM[adrm + ch]);
            }
            Sim_fprintf(fp, "  Status= %01x %01x %01x %01x\n",
...
void Put_Port_Status(FILE *fp)
{
    ...
    Sim_fprintf(fp, "CPU TEST Pin = %01x\n", TEST);
    //
    Sim_fprintf(fp, "ROM Port Input  (chip : 0~15) =");
    for (chip = 0; chip < 16; chip++)
    {
        Sim_fprintf(fp, " %01x", ROM_PORT_IN[chip]);
    }
    Sim_fprintf(fp, "\n");
    //
    Sim_fprintf(fp, "ROM Port Output (chip : 0~15) =");
    for (chip = 0; chip < 16; chip++)
    {
        Sim_fprintf(fp, " %01x", ROM_PORT_OUT[chip]);
    }
    Sim_fprintf(fp, "\n");
    //
    for (bank = 0; bank < 8; bank++)
    {
        Sim_fprintf(fp, "RAM Port Output Bank = %d (chip : 0~3) =", bank);
        for (chip = 0; chip < 4; chip++)
        {
            Sim_fprintf(fp, " %01x", RAM_PORT_OUT[bank * 4 + chip]);            
        }
        Sim_fprintf(fp, "\n");
    }
}
...
//==========================
const uint8_t *SIMCMD = "### Q/R/G adr/S/M bnk/P >";
