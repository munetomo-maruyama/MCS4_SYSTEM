...
void Wait_mSec(uint32_t mSec)
{
    ...
    while (1)
    {
        ...
        if (cycle_now >= cycle_prev + ((uint64_t)mSec * (uint64_t)freq / 1000LL)) break;
    }
}
...
void Wait_uSec(uint32_t uSec)
{
    ...
    while (1)
    {
        ...
        cycleh = (uint64_t) read_csr(mcycleh);
        cycle_now = (cycleh << 32) + (cyclel);
        if (cycle_now >= cycle_prev + ((uint64_t)uSec * (uint64_t)freq / 1000000LL)) break;
    }
}
...
void Wait_Cycle(uint64_t cyc)
{
    ...
    while (1)
    {
        ...
        cycleh = (uint64_t) read_csr(mcycleh);
        cycle_now = (cycleh << 32) + (cyclel);
        if (cycle_now >= (cycle_prev + cyc)) break;
    }
}
