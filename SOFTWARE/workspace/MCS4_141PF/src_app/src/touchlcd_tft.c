...
//-----------------------------------------------------
// LCD Draw a Character aligned at Font Scale Size
//-----------------------------------------------------
// scale should be 0, 1 or 2
void LCD_Draw_Char(uint8_t ch, uint16_t posx, uint16_t posy,
                   uint16_t color_f, uint16_t color_b, uint32_t scale)
{
    ...
    ch = (ch < 0x20)? 0x20 : (ch > FONT_LAST)? FONT_LAST : ch;
    //
...
//----------------------------------------
// LCD Draw a Character at any position
//----------------------------------------
// scale should be 0, 1 or 2
void LCD_Draw_Char_Any_Pos(uint8_t ch, uint16_t posx, uint16_t posy,
                   uint16_t color_f, uint16_t color_b, uint32_t scale)
{
    uint16_t x0, y0;
    uint16_t xsize, ysize;
...
//-------------------------------------------------
// LCD_Draw_String aligned at Font Scale Size
//-------------------------------------------------
void LCD_Draw_String(uint8_t *pStr, uint16_t posx, uint16_t posy,
                     uint16_t color_f, uint16_t color_b, uint32_t scale)
{
    uint16_t x0, y0;
    uint16_t xsize, ysize;
...
//-------------------------------------------------
// LCD_Draw_String at any position
//-------------------------------------------------
void LCD_Draw_String_Any_Pos(uint8_t *pStr, uint16_t posx, uint16_t posy,
                     uint16_t color_f, uint16_t color_b, uint32_t scale)
{
    uint16_t x0, y0;
    uint16_t xsize, ysize;
    ...
    while(*pStr != '\0')
    {
        LCD_Draw_Char_Any_Pos(*pStr, posx, posy, color_f, color_b, scale);
        pStr++;
        posx = posx + xsize;
        if ((posx + xsize) > LCD_WIDTH)
        {
            posx = 0;
            posy = posy + ysize;
            if ((posy + ysize) > LCD_HEIGHT)
            {
                posy = 0;
            }
        }
