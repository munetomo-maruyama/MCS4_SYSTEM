...
void GPIO_Init(void)
{
    ...
    mem_wr32(GPIO_PDR3, 0x00000000);
    mem_wr32(GPIO_PDR4, 0x00000000);
    mem_wr32(GPIO_PDR5, 0x00000000);
    //
    mem_wr32(GPIO_PDD0, 0xffffffff);
    mem_wr32(GPIO_PDD1, 0x1fffffff);
    mem_wr32(GPIO_PDD2, 0x00000000);
    mem_wr32(GPIO_PDD3, 0xffffffff);
    mem_wr32(GPIO_PDD4, 0x00000000);
    mem_wr32(GPIO_PDD5, 0xffffff00);
    //
    GPIO_SetLED(0);
    GPIO_PutCharSEG(' ');
}
...
//----------------------------
// Put Char in 7-Segment LED
//----------------------------
void GPIO_PutCharSEG(uint8_t ascii)
{
    uint32_t i;
    uint8_t  led;
    static uint32_t seg[6] = {LED_SPC, LED_SPC, LED_SPC, LED_SPC, LED_SPC, LED_SPC};
    //
    led = (ascii < 128)? LED7SEG_ASCII[ascii] : LED_SPC;
    //
    for (i = 0; i < 6; i++)
    {
        if (i < 5)
    ...
    //
    uint32_t out;
    out = (seg[3] << 24) | (seg[2] << 16) | (seg[1] << 8) | (seg[0] << 0);
    mem_wr32(GPIO_PDR0, out);
    out = mem_rd32(GPIO_PDR1);
    out = out & 0xffff0000;
    out = out | (seg[5] << 8) | (seg[4] << 0);
    mem_wr32(GPIO_PDR1, out);
}
