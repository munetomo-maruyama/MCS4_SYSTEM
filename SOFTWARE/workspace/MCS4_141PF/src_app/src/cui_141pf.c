...
{
    // Create Mutex
    xMutex_CUI_UART = xSemaphoreCreateMutex();
    xMutex_CUI_GPIO = xSemaphoreCreateMutex();
    // Create Binary Semaphore
    xSemaphore_CUI = xSemaphoreCreateBinary();

    // Create Tasks
    if ((xMutex_CUI_UART != NULL) && (xMutex_CUI_GPIO != NULL) &&(xSemaphore_CUI))
    {
        xTaskCreate(Task_CUI_141PF_Key, "KEY", configMINIMAL_STACK_SIZE * 2U, NULL,
                PRIORITY_TASK_CUI_141PF_KEY, NULL );
        xTaskCreate(Task_CUI_141PF_Prt, "PRT", configMINIMAL_STACK_SIZE * 2U, NULL,
                PRIORITY_TASK_CUI_141PF_PRT, NULL );
    }
...
void External_Interrupt_Handler(void)
{
    ...
    if (INT_UART_Check())
    {
        ...
        xSemaphoreGiveFromISR(xSemaphore_CUI, &xHigherPriorityTaskWoken);
    }
    //
...
static void Task_CUI_141PF_Key(void *pvParameters)
{
    ...
    for ( ;; )
    {
        ...
        if (state != 1)
        {
            ...
            xSemaphoreTake(xSemaphore_CUI, portMAX_DELAY);
            //
            // Take Mutex_UART
            xStatus = xSemaphoreTake(xMutex_CUI_UART, xTicksToWait);
            //
            if (xStatus == pdTRUE)
            {
            ...
            // Release Mutex_UART
            xSemaphoreGive(xMutex_CUI_UART);
        }
        // Handle Key Command
        MCS4_CUI_141PF_Key_Control(&state, rxd);
        //
        // Take Intervals
...
static void Task_CUI_141PF_Prt( void *pvParameters )
{
    ...
    {
        // Handle Printer
        MCS4_CUI_141PF_Prt_Control(&state);
        //
        // Take Intervals
        vTaskDelay(pdMS_TO_TICKS(50));
    }
...
void MCS4_CUI_141PF_Key_Control(uint32_t *pState, uint8_t rxd)
{
    ...
    switch(*pState)
    {
        ...
        {
            // Print 7-Seg LED
            xStatus = xSemaphoreTake(xMutex_CUI_GPIO, xTicksToWait);
            if (xStatus == pdTRUE)
            {
                GPIO_PutCharSEG(rxd);
            }
            xSemaphoreGive(xMutex_CUI_GPIO);
            //
            // Make Key Code
            switch(rxd)
            {
                case C_KEY_S   : {cui_keycode = 0x09d; break;} // S  Sign (for Negative Input)
                case C_KEY_EX  : {cui_keycode = 0x09e; break;} // EX Exchange (for Division)
                case C_KEY_CE  : {cui_keycode = 0x09f; break;} // CE Clear Entry
                case C_KEY_C   : {cui_keycode = 0x0a0; break;} // C  Clear All
                //
                case C_KEY_7   : {cui_keycode = 0x099; break;} // 7
                case C_KEY_4   : {cui_keycode = 0x09a; break;} // 4
                case C_KEY_1   : {cui_keycode = 0x09b; break;} // 1
                case C_KEY_0   : {cui_keycode = 0x09c; break;} // 0
                //
                case C_KEY_8   : {cui_keycode = 0x095; break;} // 8
                case C_KEY_5   : {cui_keycode = 0x096; break;} // 5
                case C_KEY_2   : {cui_keycode = 0x097; break;} // 2
                case C_KEY_00  : {cui_keycode = 0x098; break;} // 00
                //
                case C_KEY_9   : {cui_keycode = 0x091; break;} // 9
                case C_KEY_6   : {cui_keycode = 0x092; break;} // 6
                case C_KEY_3   : {cui_keycode = 0x093; break;} // 3
                case C_KEY_DOT : {cui_keycode = 0x094; break;} // .
                //
                case C_KEY_SUB : {cui_keycode = 0x08d; break;} // -
                case C_KEY_ADD : {cui_keycode = 0x08e; break;} // +
                //
                case C_KEY_NUM : {cui_keycode = 0x089; break;} // #  Print Again
                case C_KEY_DIV : {cui_keycode = 0x08a; break;} // /
                case C_KEY_MUL : {cui_keycode = 0x08b; break;} // X
                case C_KEY_EQU : {cui_keycode = 0x08c; break;} // =
                //
                case C_KEY_SQR : {cui_keycode = 0x085; break;} // SQ Square Root
                case C_KEY_PER : {cui_keycode = 0x086; break;} // %  Percent
                case C_KEY_MES : {cui_keycode = 0x087; break;} // M=- After Equal, Memory Sub
                case C_KEY_MEA : {cui_keycode = 0x088; break;} // M=+ After Equal, Memory Add
                //
                case C_KEY_MC  : {cui_keycode = 0x081; break;} // MC  Memory Clear
                case C_KEY_MR  : {cui_keycode = 0x082; break;} // MR  Memory Read
                case C_KEY_MS  : {cui_keycode = 0x083; break;} // M-  Memory Sub
                case C_KEY_MA  : {cui_keycode = 0x084; break;} // M+  Memory Add
                //
                default : {cui_keycode = 0x000; break;}
            }
            //
            // Paper Feed?
            cui_feed = (rxd == C_KEY_FED)? 1 : 0;
            //
            // Escape?
            ...
            if (rxd == ESC)
            {
                ...
                {
                    printf("----------------------------\n");
                    printf("Set Slide Switches on 141-PF\n");
                    printf("----------------------------\n");
                    printf("Decimal Point : 0~6/8 (now %1x)=? ", cui_decimal);
                }
                xSemaphoreGive(xMutex_CUI_UART);
                *pState = 2;
            }
            ...
            else
            {
                port_key_prt_cmd = (1            << 31)  // enable 141-PF
                                 | (cui_feed     << 14)  // paper feed
                                 | (cui_rounding << 12)  // rounding switch
                                 | (cui_decimal  <<  8)  // decimal point
                                 | (cui_keycode  <<  0); // key code
        ...
        case 2:
        {
            ...
            if (xStatus == pdTRUE)
            {
                ...
                {
                    cui_decimal = (uint32_t)(rxd - '0');
                    printf("Decimal Point SW Changed (now %1x)\n", cui_decimal);
                }
                else
                {
                    printf("Decimal Point SW Not Changed (now %1x)\n", cui_decimal);
                }
                //
                printf("Rounding : FLOT=0/ROUN=1/TRNC=8 (now %1x)=? ", cui_rounding);
            }
            xSemaphoreGive(xMutex_CUI_UART);
            *pState = 3;
            break;
        ...
        case 3:
        {
            ...
            if (xStatus == pdTRUE)
            {
                ...
                {
                    cui_rounding = (uint32_t)(rxd - '0');
                    printf("Rounding SW Changed (now %1x)\n", cui_rounding);
                }
                else
                {
                    printf("Rounding SW Not Changed (now %1x)\n", cui_rounding);
                }
                printf("----------------------------\n");
            }
            xSemaphoreGive(xMutex_CUI_UART);
...
void MCS4_CUI_141PF_Prt_Control(uint32_t *pState)
{
    ...
    static uint8_t pline[CUI_PLINE_MAX] =
    {
        ' ',' ',' ',' ',' ',
        ' ',' ',' ',' ',' ',
        ' ',' ',' ',' ',' ',
        ...
        case 1 :
        {
            ...
            if (fifo_pop_ack)
            {
                ...
                if (prt_feed)
                {
                    ...
                    for (i = 0; i < CUI_PLINE_MAX - 1 ; i++) pline[i] = ' ';
                }
                // Red Ink?
                else if (prt_red)
                {
